...
test('red team: place and memory persist and trip deletion cascades', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Add your first city', exact: true }).click()
  await page.getByRole('button', { name: 'Use online city lookup' }).click()
  ...
  await page.getByRole('tab', { name: 'Cities & places', exact: true }).click()
...
test('red team: mobile trip carousel opens plan and journal remains accessible', async ({ page }) => {
  ...
  await page.getByRole('button', { name: /CITY 1 Hanoi/ }).click()
  await expect(page.getByRole('tab', { name: 'Cities & places' })).toHaveAttribute('aria-selected', 'true')
  await expect(page.getByRole('heading', { name: 'Hanoi', exact: true })).toBeVisible()
  await page.getByRole('tab', { name: 'Notes', exact: true }).click()
  await expect(page.getByRole('heading', { name: 'The city wakes before us', exact: true })).toBeVisible()
  await page.getByRole('tab', { name: 'Cities & places' }).click()
  await page.getByRole('button', { name: 'Add city', exact: true }).click()
  await expect(page.getByLabel('City name', { exact: true })).toBeVisible()
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true)
})
